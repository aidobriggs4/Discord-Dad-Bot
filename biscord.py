...
async def on_message(message):
    ...
    if message.content.startswith(keybind + "gtogglemute"): # Mute the bot globally (depreciated, might not mute all bot functions)
        if str(message.author.id) in admin:
            ...
            await message.channel.send(embed=embed)
    if message.content.startswith(keybind + "MuteMe"): # Mute the bot globally (depreciated, might not mute all bot functions)
            if muted == "False":
                print('muted')
                with open('config.ini', 'w') as configfile:
                    config.read('config.ini')
                    config.set('settings', 'blacklist', config.get('settings', 'blacklist') + " ," + str(message.author.id) + ((message.author.name)))
                    config.write(configfile)
                    embed=discord.Embed(title="Dad", color=0x0000ff, description="Dad is now muted!")
                    await message.channel.send(embed=embed)
            elif muted == "True":
    ...
    if message.content.startswith(keybind + "help"): # Help!!! dick stuck in toaster
        ...
        embed.add_field(name="MuteMe", value="Mutes the user", inline=False)
